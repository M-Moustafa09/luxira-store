...
- **🔒 قاعدة صارمة: مفيش commit مباشر على `main` أبداً — لا فيتشر، لا bug fix، ولا حتى تعديل سطر واحد.** كل حاجة تاخد branch منفصل + PR workflow، من غير استثناء بسبب "الحجم صغير". تفاصيل كاملة وسبب التأكيد الصريح في قسم "سياسة الـ Merge" تحت.
...
### Phase 3, Module A — Negative Comment Filtering + Daily Review Stats ✅
أول موديول من دفعة تالتة جاية من المدير (Phase 3، 2026-08-16)، مقسّمة بقرار من المستخدم لـ 4 موديولات منفصلة (A/B/C/D)، كل واحد بيتحلل ويتخطط لوحده بعد ما اللي قبله يتعمله merge — مفيش تخطيط لـ B قبل ما A يخلص. Module A نفسه شمل مهمتين مرتبطين (فلترة التعليقات السلبية + عداد يومي إيجابي/سلبي) لأن التانية بتعتمد على نفس منطق التصنيف بتاع الأولى.

- **القرارات اللي اترجعت من المستخدم على الأسئلة التوضيحية**: الكشف عن "سلبي" بقايمة كلمات مفتاحية (مش تحليل مشاعر/NLP آلي — تعقيد زيادة عن اللزوم لمشكلة بسيطة). التعليق السلبي بيتحفظ لكن بيتخفى تلقائياً (`IsVisible=false`) — نفس آلية الإخفاء اليدوي الموجودة أصلاً من الأدمن، مش رفض كامل (عشان يفضل فيه سجل يقدر الأدمن يراجعه ويرجع فيه لو غلط). التصنيف الإيجابي/السلبي للعداد اليومي بيعتمد على `Rating` (>=4 إيجابي، <=2 سلبي، =3 محايد ومستبعد من الاتنين) مش عكس نتيجة فلتر الكلمات — قرار متعمد إن التقييم بالنجوم إشارة أوضح من غياب كلمة سلبية بس.
- **الداتا موديل**: `Review.IsFlaggedNegative` (bool, افتراضي `false`) — عمود جديد بيتفرّق بين تعليق اتخفى تلقائياً بسبب الفلتر وتعليق اتخفى يدوياً من الأدمن، الاتنين بيشتركوا في `IsVisible=false` لكن بس الأول بيبقى `IsFlaggedNegative=true`. Indexes جديدة على `IsFlaggedNegative` و`CreatedAt` (الأخيرة لازمة لاستعلامات "اليوم" في الإحصائيات).
- **`INegativeKeywordFilter`/`NegativeKeywordFilter`**: مطابقة substring بعد تطبيع النص العربي (حذف التشكيل والتطويل، توحيد صيغ الألف `أإآ`→`ا`، الألف المقصورة `ى`→`ي`، التاء المربوطة `ة`→`ه`) — **قيد معروف ومقصود**: صيغ الهمزة على الياء/الواو (`ئ`/`ؤ`) مش بتتوحّد مع الهمزة المجردة، عشان توحيدها كانت هتزود false positives أكتر من الفايدة. القايمة (32 كلمة/عبارة) متخزنة في `appsettings.json` تحت `Moderation:NegativeKeywords` (مش جدول DB — مفيش طلب لواجهة أدمن تدير الكلمات).
- **مراجعة القايمة قبل التفعيل**: القايمة الأصلية اللي المستخدم بعتها (39 عنصر) اتراجعت سطر بسطر قبل التفعيل، مش قُبلت زي ما هي — 4 كلمات اتشالت لخطر false-positive حقيقي وخاص بسياق متجر مستحضرات تجميل تحديداً (`حيوان` بيتطابق جوه "الحيوانات" في ادعاءات "لم يتم اختباره على الحيوانات" الشائعة والإيجابية، `تقليد` بيتطابق جوه "تقليدي" وصف إيجابي شائع للعطور، `غش` بيتطابق جوه "غشاء" مصطلح شائع في وصف مكونات العناية بالبشرة، `وحش` غامضة بين معنى "سيء" المقصود ومعنى "افتقد" الإيجابي الشائع في اللهجة المصرية) — والقايمة اتبسطت بعد كده باستخدام جذور بدل صيغ مصرّفة متكررة (`سرق` بدل `سرقة`/`سرقوني`/`سرقوا فلوسي`، `محتال` بدل `محتالين`، حذف `نصبوا` لأنها substring متضمنة أصلاً في `نصب`) — تغطية أوسع بعدد أقل من الإدخالات.
- **الـ enforcement**: `ReviewService.CreateAsync` بينادي الفلتر على النص وقت الإنشاء، ولو طابق بيحط `IsVisible=false, IsFlaggedNegative=true` على نفس الـ insert — التعليق بيتحفظ ويرجع `201` للعميل زي أي تعليق عادي، بس مبيظهرش عام ومبيتحسبش في `Product.Rating`/`ReviewsCount` (نفس مسار `RecomputeProductAggregateAsync` الموجود أصلاً).
- **`GET /api/admin/analytics/reviews`** (جديد، بيوسّع `IAnalyticsService`/`AnalyticsService` الموجودة أصلاً لـ Site Visits، نفس حدود اليوم التقويمية UTC): `NegativeBlockedToday` (عدد التعليقات اللي الفلتر خباها النهاردة)، `PositiveToday`/`NegativeToday` (حسب `Rating`، **مستقلين تماماً عن `IsVisible`/`IsFlaggedNegative`** — تصنيف بيقيس حجم التعليقات مش اللي ظاهر للعميل فعلاً، قرار اتاخد صراحة). استعلام واحد (`GetDailyClassificationStatsAsync`) بيجيب صفوف اليوم مرة واحدة ويصنّفهم في الذاكرة بدل 3 استعلامات `COUNT` منفصلة.
- **`ReviewDto.IsFlaggedNegative`** اتضاف عشان قايمة الأدمن (`GET /api/admin/reviews`) تقدر تفرّق بين تعليق اتخفى تلقائياً وتعليق اتخفى يدوياً — Mapster عمل الـ mapping تلقائياً بمجرد إضافة الحقل (نفس اسم الخاصية في الـ entity والـ DTO)، من غير config إضافي.
- **اتعمله اختبار end-to-end حي بالكامل عن طريق الـ API الفعلي الشغال** (مش unit tests بس): تعليق نضيف اترجع `isVisible:true`، تعليق فيه "نصاب"/"زبالة" اترجع `isVisible:false, isFlaggedNegative:true` واختفى من قايمة `GET /api/products/{id}/reviews` العامة ومن حساب `Product.Rating`؛ إخفاء تعليق نضيف يدوياً من الأدمن أكد إنه فضل `isFlaggedNegative:false` (يعني الاتنين فعلاً متفرّقين مش نفس الحالة)؛ زرع صفوف تقييمات بتواريخ معروفة (اليوم/إمبارح) وتأكيد إن `GET /api/admin/analytics/reviews` رجع أرقام مطابقة تماماً للحساب اليدوي (شامل تأكيد إن تعليق اتخفى تلقائياً بتقييم منخفض بيتحسب في الاتنين `NegativeBlockedToday` و`NegativeToday` مع بعض، مش الاتنين متنافيين — تصنيفين مستقلين تماماً زي ما اتقرر).
- **درس تشغيلي (مش قرار معماري)**: نص عربي مبعوت كـ inline argument لـ `curl` جوه Bash tool في بيئة Windows/Git-Bash ده بيتلخبط لعلامات استفهام حرفية (`?`) قبل ما يوصل للسيرفر أصلاً — اتأكد بفحص البايتات مباشرة (`xxd`). الحل: كتابة الـ JSON payload لملف UTF-8 حقيقي واستخدام `curl -d @file` بدل الـ inline string. لازم يتفتكر لأي اختبار API مستقبلي فيه نص عربي عن طريق `curl` في البيئة دي.

### Phase 3, Module B — Randomized Auto-Reply on New Comments ✅
تاني موديول من دفعة Phase 3 (اتخطط بعد ما Module A اتعمله merge، مش قبل كده، بقرار من المستخدم). المهمة: أي تعليق جديد على منتج لازم ياخد رد تلقائي بعد تأخير عشوائي بين دقيقة و5 دقايق.

- **القرارات اللي اترجعت من المستخدم على الأسئلة التوضيحية**: نص الرد مش رسالة واحدة ثابتة لكل فئة — قايمة (pool) من ردود متنوعة لكل فئة تقييم (إيجابي/سلبي/محايد)، بيتم اختيار واحد عشوائي منها في كل مرة (مش نفس الرد ثابت لنفس الفئة). النصوص الفعلية اتوصّلت من المستخدم (4 إيجابي، 4 سلبي، 2 محايد). آلية التأخير: **Option 1 المقترحة** — تاريخ استحقاق متخزن على صف الـ `Review` نفسه + `BackgroundService` بيعمل polling، مش timer في الذاكرة (مش بيعيش بعد إعادة تشغيل السيرفر) ولا مكتبة جدولة خارجية زي Hangfire/Quartz.NET (تعقيد زيادة عن اللزوم لجدولة نوع واحد بس من الأحداث). النطاق: رد تلقائي بس، مفيش endpoint لرد يدوي من الأدمن دلوقتي (بس الـ entity اتصمم عشان يستحمل الإضافة دي لاحقاً من غير إعادة تصميم). Edge case: تعليق اتحجب تلقائياً (Module A) **مايجيلوش** رد تلقائي خالص — مفيش فايدة من رد على حاجة مخفية أصلاً.
- **الداتا موديل**: `ReviewReply` entity جديد وعام (`ReviewId`, `Text`, `IsAutomated`, `CreatedAt`) — مش auto-reply-only بالتصميم، عشان رد أدمن يدوي مستقبلاً يبقى مجرد صف تاني بـ`IsAutomated=false` من غير أي تعديل في الـ schema. FK على `Review` بـ`Cascade` (نفس منطق `Review→Product`). `Review.AutoReplyDueAt` (nullable) + `Review.AutoReplySent` (bool) — بيتحطوا وقت الإنشاء بس لو التعليق مش `IsFlaggedNegative` (التأخير العشوائي 60-300 ثانية عن طريق `Random.Shared`)؛ لو محجوب، بيفضلوا `null`/`false` والـ query بتاع الـ polling بيستبعدهم تلقائياً من غير أي شرط استثناء إضافي.
- **`IAutoReplyTextProvider`/`AutoReplyTextProvider`**: بيقرا قايمات `AutoReply:Positive`/`Negative`/`Neutral` من `appsettings.json` (نفس أسلوب `Moderation:NegativeKeywords`) وبيرجع عنصر عشوائي حسب فئة الـ `Rating` (>=4 / <=2 / =3، نفس حدود Module A بالظبط). فيه fallback نص ثابت لو القايمة فاضية بالغلط (دفاعي بس، مش متوقع يحصل).
- **`IAutoReplyProcessor`/`AutoReplyProcessor`**: المنطق الفعلي لكل دورة polling، **اتفصل عمداً عن الـ `BackgroundService` نفسه** عشان يفضل قابل للاختبار زي أي Service تاني في المشروع (mocked `IUnitOfWork`) بدل ما يحتاج DI scope حقيقي/timer في التست. بيجيب التعليقات المستحقة (`GetDueForAutoReplyAsync`)، بيعمل `ReviewReply` لكل واحدة (النص من `IAutoReplyTextProvider` حسب تقييم التعليق)، وبيحدّث `AutoReplySent=true`. الإضافة بتحصل عن طريق `unitOfWork.ReviewReplies.AddAsync(...)` مباشرة مش `review.Replies.Add(...)` — نفس الـ gotcha الموثق في قرار #10 (parent متتبّع + إضافة child عن طريق collection navigation = EF بيغلط في تحديد الحالة).
- **`AutoReplyBackgroundService`**: **أول background job في المشروع كله**. `BackgroundService` (built-in .NET، `Microsoft.Extensions.Hosting.Abstractions` — الـ package الوحيد الجديد المُضاف) بيعمل `PeriodicTimer` كل ~30 ثانية، بيفتح DI scope جديد كل تيك (عشان يقدر يجيب `IAutoReplyProcessor` وهو scoped)، وبيمسك أي exception في اللوب من غير ما يوقفه (بيسجلها بس ويكمل — نفس فلسفة `IEmailService`/`IGeoIpLookup`). متسجل عن طريق `AddHostedService` في `Program.cs` (composition root).
- **`ReviewDto.Replies`**: قايمة `ReviewReplyDto` (الأقدم أولاً) — مبنية على EF Core's ordered/filtered include (`.Include(r => r.Replies.OrderBy(reply => reply.CreatedAt))`) على استعلامات الـ Storefront والـ Admin الاتنين. Mapster عمل الـ mapping للـ collection المتداخلة تلقائياً بمجرد إضافة الحقل، اتأكد بالاختبار الحي مش افتراض.
- **الفرونت (`ProductReviews.jsx`)**: كل تعليق بيعرض ردوده تحته، بادج مميز ("رد فريق المتجر" + أيقونة متجر) للردود اللي `isAutomated=true`. **Polling جديد اتضاف** (`usePolling`، نفس الـ hook المستخدم في `/offers`/"طلباتي"، فاصل 25 ثانية) — قرار صريح من المستخدم إن الرد لازم يظهر لوحده على صفحة منتج مفتوحة من غير ما العميل يعمل refresh يدوي، مش بس يظهر عند الزيارة الجاية. ده الاستخدام التاني لـ polling في المشروع بعد ميزة الـ Live Updates الأصلية (Offers/TrackOrder)، بنفس الـ pattern بالظبط.
- **اتعمله اختبار end-to-end حي بالكامل عن طريق الـ API والمتصفح الفعليين**: تعليق نضيف اتبعت عن طريق الـ API الحقيقي، `AutoReplyDueAt` اتنقل يدوياً للماضي (بدل انتظار حقيقي لدقايق)، وتأكيد إن الـ `BackgroundService` الحقيقي (مش الـ processor لوحده) عمل الصف الصح بعد التيك الطبيعي بتاعه. بعد كده اختبار كامل في المتصفح: تقييم اتبعت من الفورم الفعلي على صفحة منتج، `AutoReplyDueAt` اتنقل للماضي، والصفحة **اتسابت مفتوحة من غير أي لمسة** لمدة ~40 ثانية (أكتر من دورتي الـ polling التلقائيتين — بتاعة الباك إند وبتاعة الفرونت) — لقطة شاشة بعدها أكدت إن الرد ظهر لوحده بالبادج الصح، من غير أي refresh يدوي.
- **درس عملي (استمرار من نفس الحادثة الموثقة في Module A)**: أي اختبار API بنص عربي عن طريق `curl` inline في البيئة دي لازم يستخدم `curl -d @file.json` بملف UTF-8 حقيقي — اتطبق هنا كمان من غير مفاجآت.

### Phase 3, Module C — Live Visitor Notification 🔴 مؤجل بالكامل (لسه معملش أي كود)
تالت موديول من دفعة Phase 3 (المفروض بعد Module B). **قرار صريح من المستخدم**: الموديول ده متأجل بالكامل — مش بس التحقق النهائي زي حالات تانية، **ولا سطر كود واحد يتكتب دلوقتي** — لحد ما فريق الفرونت يشارك كود الـ Admin Dashboard الفعلي.

- **السبب**: قبل ما نبدأ التحليل، اتوضح إن الـ Admin Dashboard لسه repo منفصل مالوش وصول ليه (زي ما هو موثق من زمان في قسم "نقاط لسه معلقة" فوق) — يعني أي إشعار "real-time على الداشبورد" مينفعش يتبني أو يتأكد منه end-to-end من غيره. بس السبب الأهم اللي خلى المستخدم يأجل حتى الـ **backend** (مش بس الفرونت): **تصميم الـ contract نفسه (SignalR hub / SSE endpoint / أي حاجة تانية) ممكن يتغير لما نشوف الـ stack والاحتياجات الفعلية بتاعة فريق الداشبورد** — بناء backend مبني على افتراضات دلوقتي ممكن يتبني غلط ويحتاج إعادة تصميم لما الكود الحقيقي يوصل.
- **التحليل اللي اتعمل قبل قرار التأجيل** (يفضل موجود هنا كمرجع لما نرجع للموضوع): اتعمل تحليل trade-offs فريش (مش نفس نتيجة SignalR-vs-Polling بتاعة الـ Live Updates الأصلية، بقرار من المستخدم بمراجعة الموضوع من الصفر) بين SignalR/SSE/Polling خصيصاً للحالة دي — الفرق الجوهري عن حالة الـ Offers banner: الجمهور هنا صغير ومصدّق عليه أصلاً (أدمن بس، JWT موجود فعلاً)، والحدث نفسه push-shaped مش state-shaped (زيارة حصلت دلوقتي، مش حالة تتقرا تاني). القيد العملي الأكبر اللي ظهر: أي مكانيزم هنختاره لازم فريق تاني (مالوش وصول لينا) ينفذ الجزء بتاعه بيه — SSE أقل عبء تكامل عليهم (مفيش package جديد، `EventSource` أصلي في المتصفح) لكن أكتر كود يدوي عندنا (مفيش framework SSE جاهز في ASP.NET Core زي SignalR)، SignalR framework كامل وجاهز عندنا لكن محتاج الفريق التاني يضيف `@microsoft/signalr` package. الاستنتاج ده اتسجل هنا **كمرجع بس** — القرار النهائي هيتاخد لما التحليل يتعاد بعد ما الكود يتشارك، مش دلوقتي.
- **سؤال مفتوح لسه متأجل هو الآخر**: هل "زائر بيتصفح دلوقتي" يعني حدث لحظي (إشعار عند بداية زيارة جديدة، بيستخدم `POST /api/analytics/visit` الموجود فعلاً) ولا roster حي مستمر ("N زائر أونلاين دلوقتي"، محتاج heartbeat/TTL tracking جديد بالكامل)؟ القرار ده هيتاخد مع باقي التحليل لما نرجع للموضوع.
- **متى نرجع للموضوع**: لما المستخدم يشارك كود الـ Admin Dashboard الفعلي. عندها التحليل هيتعاد بناءً على الـ stack الحقيقي (مش افتراضات)، وهيتحدد القرار النهائي للمكانيزم + تصميم الـ contract سوا.

### Phase 3, Module D — Live Chat Triggered by First Comment 🔴 مؤجل هو الآخر (نفس سبب Module C)
رابع وآخر موديول من دفعة Phase 3. المستخدم سأل صراحة: هل D ممكن يتخطط/يتبني بشكل مستقل عن C ولا لازم ينتظر هو الآخر؟ الإجابة الصادقة (اتقالت للمستخدم قبل ما يقرر، مش قرار اتاخد من غير عرض الأسباب): **لأ، D لازم ينتظر كمان، ولأسباب أقوى من مجرد "C لسه مأجل"**:

1. **الـ spec نفسه بيربطهم**: طلب المدير الأصلي لـ Module D بيقول صراحة "real-time delivery reusing whatever we build in Module C" — يعني D اتصمم من الأساس على افتراض إن آلية الـ real-time بتاعة C موجودة وجاهزة يتستخدم منها، مش إنه هيجيب آلية منفصلة بتاعته. بناء آلية realtime جديدة دلوقتي خصيصاً لـ D كان هيبقى إما تكرار (لو C لاحقاً اختارت حاجة مختلفة) أو قرار C بالفعل من غير قصد (لو افترضنا حاجة معينة وبنيناها).
2. **قيد الوصول لكود الـ Dashboard أسوأ في D مش أهون**: الجزء بتاع الأدمن في C مجرد widget إشعار. الجزء بتاع الأدمن في D واجهة محادثة كاملة (قايمة رسائل، مربع كتابة، حالة المحادثة) — ده تصميم وتنفيذ حقيقي في repo لسه مالوش وصول ليه، مساحة أكبر محجوبة مش أصغر.
3. **متطلب الـ real-time بتاع D أصعب موضوعياً من C**: C محتاج push اتجاه واحد لحدث نادر ومنخفض الأهمية (فوات إشعار واحد أثناء إعادة اتصال مقبول). D محتاج تسليم رسائل bidirectional منخفض التأخير ومرتب (عميل نص جملة والرسالة ضاعت تجربة أسوأ بكتير من إشعار واحد فايت). الآلية المختارة لازم تتثبت على الحالة الأسهل (C) الأول قبل ما نوثق فيها الحالة الأصعب (D).
4. **اللي ممكن يتبني مستقل (نموذج البيانات: `Conversation`/`Message`) قليل ومالوش قيمة لوحده**: ممكن يترسم على الورق، لكن بناؤه فعلياً دلوقتي هيطلع كود مش متكامل ومالوش آلية real-time يتوصل بيها ولا واجهة أدمن تستقبله — بالظبط النوع من "التنفيذ الناقص" اللي مبادئ المشروع بتحذر منه صراحة (أول الملف: "مفيش تنفيذ ناقص").

**قرار المستخدم**: D مؤجل بالكامل هو الآخر، لنفس trigger بتاع C — لما فريق الفرونت يشارك كود الـ Admin Dashboard. عندها C وD هيتعاد التخطيط ليهم مع بعض في pass واحد، مش منفصلين، لأن تصميم D فعلياً بيعتمد على نتيجة تحليل C.

### Currency Label Bug Fix + Dev Country Override ✅
اتكشف أثناء ما المستخدم كان بيطلب يختبر ميزة Country Pricing (MaxMind GeoIP) محلياً، مش من طلب صريح مسبق — الـ agent لاحظه أثناء التحقق الحي وأبلغ عنه قبل ما يصلحه.

- **المشكلة**: رمز العملة "ر.س" (ريال سعودي) كان **متكتوب كنص حرفي ثابت** في كل مكان بيعرض سعر تقريباً (`ProductHero.jsx`, `Cart.jsx`, `Checkout/OrderSummary.jsx`, `Price.jsx` المشترك، وكروت المنتجات المختلفة) — بغض النظر عن قيمة حقل `currency` اللي الـ API بيرجعه فعلياً. يعني حتى لو الرقم بتاع السعر كان بيتغير صح حسب دولة الزائر (Country Pricing شغالة صح من ناحية الأرقام)، الوحدة المعروضة جنبه كانت دايماً "ر.س" غلط لأي عملة تانية غير SAR.
- **الحل**: `src/lib/currency.js` جديد — mapping من كود العملة لِلabel العربي المختصر (نفس فكرة `CountryCurrency.cs` في الباك إند، 16 عملة + USD fallback). اتوصل في كل مكان فيه حقل `currency` فعلي متاح: `Price.jsx` (المكوّن المشترك اللي بيغطي `ProductCard`/`ProductGridCard`/`ProductInfo`/`ProductCardOffers` كلهم مرة واحدة)، `ProductHero.jsx`, `RelatedProductCard.jsx`, `BestSellerCard.jsx`, `NewProductsCard.jsx` (كلهم بيستخدموا `product.currency`)، و`Cart.jsx`/`Checkout/OrderSummary.jsx` (بيستخدموا `cart.currency` — عملة السلة الموحدة زي قاعدة "الكل أو لا حاجة" الموثقة في Country Pricing).
- **الباقات (Bundles) اتسابت زي ما هي عمداً**: `BundleDto` مفهوش حقل `Currency` خالص (بقرار سابق، الباقات برّه نطاق Country Pricing) — فمفيش بيانات حقيقية يتصلح بيها `BundlesCard.jsx`/`BundleCard.jsx`، اتسابوا بـ"ر.س" الثابتة عن قصد بدل ما يتم تخمين عملة مش موجودة فعلاً في الـ response.
- **Dev Country Override (فرونت)**: اتكشف إن الباك إند عنده أصلاً آلية `?country=<Country enum name>` / `X-Dev-Country` header للتحايل على الـ IP lookup في Development (موثقة قبل كده في قسم "Country Pricing")، بس الفرونت مكانش بيبعتها خالص. `src/lib/devCountry.js` جديد (dev-only، `import.meta.env.DEV`-gated) بيقرا `?country=` من الـ URL مرة واحدة وبيخزنها في `sessionStorage` عشان تفضل شغالة عبر التنقل جوه الـ SPA من غير ما تتكرر في كل لينك، و`apiClient.js` بقى يبعتها كـ `X-Dev-Country` header مع كل request.
- **اتعمله اختبار end-to-end حي بالكامل عن طريق الـ API والمتصفح الفعليين**: زرع سعر تجريبي لمصر (EGP) على منتج، مسح الـ storage لضيف جديد، فتح صفحة المنتج بـ `?country=Egypt` — أكد إن السعر عرض "ج.م" مش "ر.س". إضافة للسلة عن طريق الـ API الحقيقي أكدت `cart.currency: "EGP"`، وصفحة السلة الفعلية عرضت "ج.م" على كل سطر (المنتج، المجموع الفرعي، الخصم، الشحن، الإجمالي). بيانات الاختبار (`ProductCountryPrice` وسطر السلة) اتنضفت بعد كده.
...
بالترتيب من الأكتر حرجاً: **(1)** ~~Admin API~~ **اتعمل ✅ بالكامل** (Auth wiring + Orders + Products + Country Pricing + Categories/Brands + Image Upload + Coupons/Bundles/Campaigns/Testimonials — مفيش Module متبقي) → **(2)** Payment Gateway (الـ checkout مش بياخد فلوس فعلياً، منتظر قرار المدير) → **(3)** ~~Rate Limiting على `/auth/*` + HSTS~~ **اتعمل ✅** → **(4)** ~~Tests + CI~~ **اتعمل ✅** (Deployment استُثنيت عن قصد، تفاصيل تحت). ~~ربط الـ Auth بالفرونت~~ **اتعمل ✅**. ~~تتبع الطلب مجمّد~~ **اتعمل ✅** (عن طريق Admin Orders). ~~Module 3c: Cart/Checkout/Order يستخدموا سعر الدولة~~ **اتعمل ✅**. ~~Stock/Inventory~~ **اتعمل ✅**. ~~Image Upload~~ **اتعمل ✅**. ~~Storefront Live Updates (Polling)~~ **اتعمل ✅** (طلب من المدير، مش من ضمن الترتيب الأصلي). ~~Phase 1 Manager Batch (Customer Blocking + Order Notifications + Visit Analytics)~~ **اتعمل ✅** (طلب من المدير كمان، مش من ضمن الترتيب الأصلي — تفاصيل في قسم "Phase 1 — Manager Batch" فوق). ~~Phase 2: Product Reviews (Comments System)~~ **اتعمل ✅** (طلب من المدير كمان، تفاصيل في قسم "Product Reviews (Comments System)" فوق). ~~Phase 3 Module A: Negative Comment Filtering + Daily Review Stats~~ **اتعمل ✅** (أول موديول من 4 في دفعة Phase 3، تفاصيل في قسم "Phase 3, Module A" فوق). ~~Phase 3 Module B: Randomized Auto-Reply on New Comments~~ **اتعمل ✅** (تاني موديول من 4، تفاصيل في قسم "Phase 3, Module B" فوق). **🔴 Phase 3 Module C: Live Visitor Notification — مؤجل بالكامل بقرار صريح من المستخدم** (لا backend ولا frontend، لحد ما فريق الفرونت يشارك كود الـ Admin Dashboard — تفاصيل كاملة في قسم "Phase 3, Module C" فوق). **🔴 Phase 3 Module D: Live Chat Triggered by First Comment — مؤجل هو الآخر لنفس الـ trigger** (الـ spec الأصلي بتاعه بيعتمد صراحة على آلية real-time بتاعة C، وجزء الأدمن فيه — واجهة محادثة كاملة — محتاج وصول لكود الـ Dashboard أكتر من C مش أقل — تفاصيل كاملة في قسم "Phase 3, Module D" فوق). **البنود الوحيدة المتبقية دلوقتي من الأولويات الأصلية**: **(2) Payment Gateway** و**Deployment** — الاتنين معلقين بانتظار قرارات من المستخدم (اختيار الـ gateway، واختيار الـ hosting target)، مفيش حاجة تانية نقدر ننفذها فيهم من غير المستخدم. اللغة (Task 2) وOption C (guest cart merge on login) أقل حرجاً من دول التاني — مفيش منهم بيمنع عميل يتصفح/يسجل/يشتري.

### حالة الـ Branches
- `main` — up to date مع origin لحد آخر push (commit `33aaaec`، Currency Label Bug Fix + Dev Country Override، اتعمله merge عن طريق GitHub PR #17، وCI اتأكد إنه نجح فعلياً على الـ merge commit نفسه). كل الـ storefront modules، Bundle→Cart، Cart notifications، تأكيد حذف من السلة، Auth backend + frontend، Admin API كامل، Stock/Inventory، Variant Upsert Fix، Live Updates Polling، Rate Limiting + HSTS، Unit Tests + CI، Phase 1 Manager Batch، Phase 2 Product Reviews، Phase 3 Module A + B، Phase 3 Module C/D deferral docs، Currency Label Fix — كلهم متعملهم merge وموجودين ومدفوعين لـ origin.
- كل الـ feature branches السابقة (`feature/auth`, `feature/cart-notifications`, `feature/cart-remove-confirm`, `feature/auth-frontend`, `feature/bundle-to-cart`, `feature/admin-api-country-pricing`, `feature/admin-categories-brands`, `feature/stock-inventory`, `fix/product-variant-update-fk`, `feature/admin-image-upload`, `feature/admin-coupons-bundles-campaigns-testimonials`, `feature/storefront-live-updates-polling`, `feature/auth-rate-limiting-hsts`, `feature/backend-unit-tests-ci`, `feature/customer-blocking`, `feature/order-email-and-notifications`, `feature/site-visit-analytics`, `feature/product-reviews`, `feature/negative-comment-filtering`, `feature/auto-reply-comments`, `feature/currency-label-fix`, `docs/status-update`, `docs/auth-status-update`, `docs/stock-status-update`, `docs/variant-fk-fix-status-update`, `docs/image-upload-status-update`, `docs/admin-crud-status-update`, `docs/live-updates-polling-status-update`, `docs/rate-limiting-hsts-status-update`, `docs/tests-ci-status-update`, `docs/phase1-manager-batch-status-update`, `docs/phase2-reviews-status-update`, `docs/module-a-negative-filtering-status-update`, `docs/module-b-auto-reply-status-update`, `docs/module-c-deferred-status-update`) اتعملها merge بالكامل لـ `main` ومفيش commits قدامها. مفيش شغل معلق على branch منفصل دلوقتي (غير الـ `docs/currency-fix-and-branch-policy-update` الحالي بتاع التحديث ده نفسه).
...
- **🔒 قاعدة صارمة (2026-08-16، بقرار صريح من المستخدم)**: **مفيش أي تعديل — كبير أو صغير، feature أو bug fix أو حتى سطر واحد — يتعمله commit مباشر على `main`**. كل حاجة، من غير استثناء، لازم تاخد branch منفصل بتاعها (`feature/...` أو `fix/...` أو `docs/...`) وتعدي بنفس الـ PR workflow (push → PR → مراجعة المستخدم → merge على GitHub → تأكيد الـ CI على الـ merge commit → sync). القاعدة دي كانت متبعة عملياً من زمان لكل الـ features، لكن اتأكدت صراحة كقاعدة صارمة بعد fix صغير (currency label bug) عشان يتوضح إنها مش مقصورة على "الشغل الكبير" بس.
...
**23. Phase 3 Module A — فلترة الكلمات مبنية على كلمات مفتاحية فقط، والتصنيف اليومي مستقل تماماً عن الظهور (تفاصيل كاملة في قسم "Phase 3, Module A" فوق):**
- الكشف عن "سلبي" قرار متعمد إنه substring matching بسيط بدل أي تحليل مشاعر آلي — نفس مبدأ "ممنوع التعقيد الزيادة عن اللزوم" في أول الملف، ومطابق لما المستخدم أصلاً اقترحه.
- **قايمة الكلمات لازم تتراجع مش تتقبل زي ما هي**: أي كلمة مفتاحية مقترحة لازم تتفحص ضد سياق المتجر تحديداً (مستحضرات تجميل) قبل ما تتضاف — كلمات عامة زي "حيوان"/"تقليد" ممكن تتصادم مع لغة تسويقية إيجابية شائعة (cruelty-free claims، "عطر تقليدي"). الدرس ده لازم يتطبق على أي قايمة كلمات مفتاحية تانية تتضاف مستقبلاً (Module B مثلاً لو احتاج نص رد تلقائي).
- تعليق اتخفى تلقائياً بيفضل بيتحسب في `NegativeToday` (لو تقييمه منخفض) وفي `NegativeBlockedToday` مع بعض — التصنيفين مش متنافيين، دول إجابتين لسؤالين مختلفين تماماً (هل اتحجب؟ / هل تقييمه سلبي؟) بقرار متعمد من المستخدم.
- درس تشغيلي مهم لأي اختبار مستقبلي: نص عربي كـ `curl` inline argument في بيئة Windows/Git-Bash بيتلخبط لـ `?` حرفية — لازم `curl -d @file.json` بملف UTF-8 حقيقي بدل الـ inline string.

**24. Phase 3 Module B — أول background job في المشروع، والـ business logic منفصل عن الـ scheduling عمداً (تفاصيل كاملة في قسم "Phase 3, Module B" فوق):**
- القرار بين persisted due-time + polling، timer في الذاكرة، أو مكتبة جدولة خارجية اتاخد بعد عرض الـ trade-offs الثلاثة على المستخدم صراحة (نفس أسلوب SignalR-vs-Polling وGmail-vs-Brevo قبل كده) — مش افتراض. الـ polling (نفس pattern الـ Live Updates الأصلية) كسب لأنه الوحيد اللي بينجو من إعادة تشغيل السيرفر من غير أي dependency جديدة.
- `AutoReplyBackgroundService` (الـ `BackgroundService` نفسه) اتقسم عمداً عن `AutoReplyProcessor` (الـ business logic) — مش لازم كده لكل background job في المشروع، لكن هنا كان مهم عشان الـ processor يفضل قابل للاختبار بنفس أسلوب باقي الـ Services (`IUnitOfWork` mocked) بدل ما يحتاج DI scope حقيقي جوه التست. أي background job جديد مستقبلاً لازم يتبع نفس الفصل ده.
- `ReviewReply` entity اتصمم عام (`IsAutomated` bool) مش auto-reply-only، من غير ما يتطلب فعلياً بناء أي حاجة تانية غير الـ auto-reply دلوقتي — قرار "صمم عشان يوسّع من غير ما تبنيه دلوقتي" مشابه لقرار `IPaymentGateway`/`IStorageService` قبل كده.
- الـ polling على صفحة تفاصيل المنتج (فرونت) قرار صريح من المستخدم بعد ما اتسأل تحديداً — الـ spec الأصلي "لازم يظهر" كان ممكن يتفسّر إنه كافي يظهر عند الزيارة الجاية بس، فاتوضح الفرق واتقرر إن العميل قاعد على صفحة مفتوحة لازم يشوفه لوحده من غير refresh، بنفس الـ pattern المستخدم أصلاً في Offers/"طلباتي".
